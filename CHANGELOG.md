...
- Add experimental `newton.solvers.SolverBoundaryImpulse` coupling a reduced-coordinate subsystem (e.g. `SolverFeatherstone`) to a maximal-coordinate subsystem (e.g. `SolverKamino`) through a single rigid 6D boundary impulse, feeding the equal-and-opposite reaction to both sides; the boundary solve runs on-device so the coupled step is CUDA-graph capturable. See `example_boundary_impulse_toy.py` and `docs/experimental_boundary_impulse.md`.
