...
- Add optional probed effective-inertia coupling to experimental `newton.solvers.SolverBoundaryImpulse` (`Config.use_effective_inertia`): the boundary Schur system uses the maximal subsystem's exact effective inverse inertia at the seam, measured each frame with unit-impulse probes through a separate `probe_solver`, instead of the free-base approximation; the correctly-sized impulse holds the weld (notably the angular component) without leaning on the velocity-Baumgarte stabilization.
...
- Add experimental `newton.solvers.SolverBoundaryImpulse` coupling a reduced-coordinate subsystem (e.g. `SolverFeatherstone`) to a maximal-coordinate subsystem (e.g. `SolverKamino`) through a single rigid 6D boundary impulse, feeding the equal-and-opposite reaction to both sides; the boundary solve runs on-device so the coupled step is CUDA-graph capturable. Optional `Config.max_boundary_force`, `max_boundary_torque`, and `max_reduced_correction` caps bound the transmitted wrench and the per-step arm velocity correction so a dynamic overload (e.g. lifting a grasped payload) under-corrects gracefully instead of whipping the arm. See `example_boundary_impulse_toy.py` and `docs/experimental_boundary_impulse.md`.
- Add experimental `example_franka_2f85_benchmark.py` comparing the boundary-impulse hybrid (Franka FR3 arm in `SolverFeatherstone` + Robotiq 2F-85 gripper in `SolverKamino`) against full-system `SolverKamino` and `SolverMuJoCo` baselines on speed and quality.
- Add experimental `example_franka_2f85_stacking.py` running an IK-driven Franka FR3 + Robotiq 2F-85 cube pick-and-place under three solver configurations (`mujoco`, `kamino`, and the boundary-impulse `hybrid`) with a CUDA-graph-captured substep loop, plus `example_franka_2f85_stacking_record.py` to record an MP4 per solver.
